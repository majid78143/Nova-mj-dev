...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  const overview = overviewQuery.data ?? demoOverview;
  const channels = channelsQuery.data?.length ? channelsQuery.data : demoChannels;
...
function HomePage() {
  ...
  const overview = overviewQuery.data ?? demoOverview;
  const giveaways = giveawayQuery.data?.length ? giveawayQuery.data : demoGiveaways;
...
function ChatPage() {
  ...
  const messages = [...(messagesQuery.data?.length ? messagesQuery.data : demoMessages.filter((item) => item.channelId === channelId)), ...localMessages.filter((item) => item.channelId === channelId)];
  const sendMessage = () => {
